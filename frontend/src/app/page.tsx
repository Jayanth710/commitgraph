...
import { AlertTriangle, CheckCircle, Clock, ArrowUpRight, ArrowDownLeft, Mail, Waypoints } from "lucide-react";
...
export default function DashboardPage() {
  ...
  const totalCommitments = (stats?.open_count ?? 0) + (stats?.completed_count ?? 0);
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20">
      ...
      <p className="text-gray-500 dark:text-gray-400 text-center max-w-md mb-8">
        ...
        We&apos;ll scan your inbox and extract promises, deadlines, and follow-ups.
      </p>
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20">
      ...
      <p className="text-gray-500 dark:text-gray-400 text-center max-w-md mb-2">
        Your email is connected. Commitments will appear here as new emails come in.
      </p>
      <p className="text-sm text-gray-400 dark:text-gray-500 text-center max-w-md">
        Send yourself a test email with a promise like &quot;I&apos;ll send the report by Friday&quot; to see it in action.
      </p>
    </div>
  );
}
...
function StatCard({
  ...
  const highlightStyles = {
    ...
    amber: "border-amber-300 dark:border-amber-800 bg-amber-50 dark:bg-amber-950",
  };
...
// Commitment row
// ---------------------------------------------------------------------------
function CommitmentRow({ commitment: c, onClick }: { commitment: any; onClick: () => void }) {
    ...
    >
      <div className="flex-1">
        ...
        <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
          ...
          {c.target_email || c.owner_email}
          {c.due_date && ` · Due ${new Date(c.due_date).toLocaleDateString()}`}
        </p>
      </div>
      <span className={`text-xs px-2 py-1 rounded-full font-medium ${statusColors[c.status] ?? "bg-gray-100 dark:bg-gray-800"}`}>
        {c.status}
      </span>
    </div>
  );
    ...
    <div className="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-800 p-5 mb-8">
      ...
      <ResponsiveContainer width="100%" height={240}>
        <BarChart data={formatted} barGap={2}>
          ...
          <Bar dataKey="outbound" name="I Owe" fill="#8b5cf6" radius={[3, 3, 0, 0]} />
          <Bar dataKey="inbound" name="Owed To Me" fill="#14b8a6" radius={[3, 3, 0, 0]} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
